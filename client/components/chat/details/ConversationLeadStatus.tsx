...
export function ConversationLeadStatus({ conversationId }: ConversationLeadStatusProps) {
  ...
  const parseBRL = (input: string) => {
    ...
    s = s.replace(/[^0-9,.-]/g, '');
    if (s.includes(',')) {
      // treat comma as decimal separator, remove thousand dots
      s = s.replace(/\./g, '');
      s = s.replace(',', '.');
    }
    const n = Number(s);
  ...
  const handleStartEditValue = () => {
    ...
    setValueInput(formatBRLValue(Number(lead.value || 0)));
  ...
  const handleSaveValue = () => {
    if (!lead) {
      // Create lead first if it doesn't exist (shouldn't happen due to UI guard)
    ...
    const numericValue = parseBRL(valueInput);
    updateLeadValue({
      leadId: lead.id,
      value: numericValue,
    });
  ...
  // Check if lead is valid (not null and has id)
  const hasValidLead = lead && lead.id;

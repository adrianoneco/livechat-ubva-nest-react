...
export function AssignmentRulesManager() {
  ...
  const handleSave = (data: any) => {
    console.log('[AssignmentRulesManager] handleSave called with:', data);
    if (data.id) {
      console.log('[AssignmentRulesManager] Calling updateRule.mutate');
      updateRule.mutate(data);
    } else {
      console.log('[AssignmentRulesManager] Calling createRule.mutate');
      createRule.mutate(data);
    }
  };

  const handleDelete = (id: string) => {
    console.log('[AssignmentRulesManager] handleDelete called with id:', id);
    if (confirm("Tem certeza que deseja excluir esta regra?")) {
      console.log('[AssignmentRulesManager] Calling deleteRule.mutate');
      deleteRule.mutate(id);
    }
  };

  const handleToggleActive = (id: string, isActive: boolean) => {
    console.log('[AssignmentRulesManager] handleToggleActive called:', { id, isActive });
